 ...
 * Low level API
 */
static int x25519_ext_montgomery_pubkey(unsigned char* public_key, const unsigned char* private_key);

static int x25519_ext_montgomery_az(unsigned char* az, const unsigned char* private_key);

static int x25519_ext_edwards_az(unsigned char* az, const unsigned char* secret_key);

static int x25519_ext_edwards_key(unsigned char* private_key, const unsigned char* secret_key);
...
struct x25519_sha512_context_t {
    mbedtls_sha512_context sha512_ctx;
};

x25519_sha512_context_t* x25519_sha512_alloc(void) {
    x25519_sha512_context_t* ctx = calloc(1, sizeof(x25519_sha512_context_t));
    if (ctx) {
...
int x25519_montgomery_getpub(x25519_public_key_t* public_key, const x25519_private_key_t* private_key) {
    return x25519_ext_montgomery_pubkey(public_key->p, private_key->p);
}
...
int x25519_montgomery_sign(
    ...
    x25519_ext_montgomery_az(az, private_key->p);
    x25519_ext_edwards_pubkey(ed_public_key, az);

    sign_bit = ed_public_key[31] & (unsigned char) 0x80;

    x25519_ext_edwards_sign(signature->s, signature->r, msg, msg_len, ed_public_key, az);
...
int x25519_montgomery_verify(
    ...
    x25519_ext_montgomery_to_edwards_pubkey(ed_public_key, public_key->p);
    ...
    return x25519_ext_edwards_verify(fixed_s, signature->r, msg, msg_len, ed_public_key);
}
...
int x25519_montgomery_key_exchange(
    ...
    return x25519_ext_montgomery_key_exchange(shared_key->p, public_key->p, private_key->p);
}
...
int x25519_edwards_getpub(x25519_public_key_t* public_key, const x25519_secret_key_t* secret_key) {
    ...
    (void) x25519_ext_edwards_key(private_key, secret_key->p);
    (void) x25519_ext_edwards_pubkey(public_key->p, private_key);
...
int x25519_edwards_key_exchange(
    ...
    (void) x25519_ext_edwards_key(curve_private_key, secret_key->p);
    (void) x25519_ext_edwards_to_montgomery_pubkey(curve_public_key, public_key->p);
    (void) x25519_ext_montgomery_key_exchange(shared_key->p, curve_public_key, curve_private_key);

    x25519_zeroize(curve_private_key, sizeof(curve_private_key));
    return 0;
}
...
int x25519_edwards_sign(
    ...
    (void) x25519_ext_edwards_az(az, secret_key->p);
    (void) x25519_ext_edwards_pubkey(public_key, az);
    (void) x25519_ext_edwards_sign(signature->s, signature->r, msg, msg_len, public_key, az);
...
int x25519_edwards_verify(
    ...
    return x25519_ext_edwards_verify(signature->s, signature->r, msg, msg_len, public_key->p);
}
...
int x25519_ext_montgomery_pubkey(unsigned char* public_key, const unsigned char* private_key) {
    (void) x25519_ext_edwards_pubkey(public_key, private_key);
    (void) x25519_ext_edwards_to_montgomery_pubkey(public_key, public_key);
    return 0;
}

int x25519_ext_montgomery_az(unsigned char* az, const unsigned char* private_key) {
    x25519_sha512(az, private_key, 32); // H(k) = (h0, h1, . . . , h2b−1)
    memmove(az, private_key, 32); // Restore k, and leave (hb, ..., hb-1) as is
    return 0;
}

int x25519_ext_edwards_az(unsigned char* az, const unsigned char* secret_key) {
    x25519_sha512(az, secret_key, 32);
    az[0] &= 248;
    az[31] &= 63;
    az[31] |= 64;
    return 0;
...
int x25519_ext_edwards_key(unsigned char* private_key, const unsigned char* secret_key) {
    unsigned char az[64];
    (void) x25519_ext_edwards_az(az, secret_key);
